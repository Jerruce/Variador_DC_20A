...
double PID_Controller_01(double pid_setpoint, double pid_feedback, uint8_t pid_reset){
    ...
    if(pid_reset){
		...
		if(i_comp > I_COMPONENT_POSITIVE_LIMIT){
			i_comp = I_COMPONENT_POSITIVE_LIMIT;
		}else if(i_comp < I_COMPONENT_NEGATIVE_LIMIT){
			i_comp = I_COMPONENT_NEGATIVE_LIMIT;
		}else{
			//Does nothing
		}
...
void PID_Controllers_Initialize(void){
	...
	PID_01_Set_Parameters(KP_BY_DEFAULT, KI_BY_DEFAULT, KD_BY_DEFAULT);
